...
class Application(Frame):
    ...
    def createWidgets(self):
        ...
        self.dingdingonLabel = Label(self, text='钉钉考勤文件路径').grid(row=3, column=0)
        self.entry = Entry(self, textvariable=self.dingdingonpath).grid(row=3, column=1)
        self.selectButton3 = Button(self, text='路径选择', command=self.select_dingdingon_path).grid(row=3, column=3)
