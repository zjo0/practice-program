...
class Application(Frame):
    ...
    def createWidgets(self):
        ...
        self.selectButton3 = Button(self, text='路径选择', command=self.select_workoff_path).grid(row=2, column=3)
        self.officeLabel=Label(self,text='考勤年份').grid(row=3,column=0)
        self.entry=Entry(self, textvariable=self.year_).grid(row=3, column=1)
        self.officeLabel=Label(self,text='考勤月份').grid(row=3,column=2)
        self.entry=Entry(self, textvariable=self.month_).grid(row=3, column=3)
        self.createButton=Button(self,text='生成汇总文件',command=lambda : unionrecord(self.officepath.get(),self.dingdingpath.get(),self.workoffpath.get(),self.year_.get(),self.month_.get())).grid(row=4)
